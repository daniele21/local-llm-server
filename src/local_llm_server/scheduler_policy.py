"""Explicit product settings for request queue admission.

Queueing is opt-in. A configured timeout applies only while waiting for runtime
admission; it is not presented as an end-to-end inference deadline.
"""
...
class RequestSchedulerSettings:
    ...
    def __post_init__(self) -> None:
        ...
        if self.default_queue_timeout_ms is not None and self.default_queue_timeout_ms < 1:
            raise ValueError("default_queue_timeout_ms must be >= 1")
        if self.default_queue_timeout_ms is not None and self.queue_capacity is None:
            raise ValueError("queue timeout requires request queue capacity")
